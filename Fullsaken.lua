...
if not isfolder("Monksaken") then
    makefolder("Monksaken")
end

if isfile("Monksaken/key.txt") then
    local file = readfile("Monksaken/key.txt")
    if file == "AloveW" then
...
if showGUI then
    ...
    Tabs.KeyTab:AddLabel({
        Text = "Monksaken Key System",
        DoesWrap = true,
        Size = 20,
    })

    Tabs.KeyTab:AddLabel({
        Text = "Get key from discord.gg/Monksaken",
        DoesWrap = true,
        Size = 17,
    })
    ...
    Tabs.KeyTab:AddKeyBox(function(ReceivedKey)
        if ReceivedKey == "AloveW" then
            writefile("Monksaken/key.txt", ReceivedKey)
            Obsidian:Unload()
            loadstring(game:HttpGet(scripturl))()
        else
